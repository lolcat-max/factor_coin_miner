################################################################################
# Factorization algorithms - AstroPhysics Solver Integration (CORRECTED)
################################################################################
...
################################################################################
# Factorization algorithms - AstroPhysics Solver Integration (VALIDATED)
################################################################################
from math import floor, ceil
from gmpy2 import mpz, mpq, mpfr, mpc
from gmpy2 import is_square, isqrt, sqrt, log2, gcd, is_prime, next_prime, primorial
...
class AstroDomain:
    """Physics-inspired optimization domain with momentum"""
    def __init__(self, name, initial_scale=10.0):
        self.name = name
        self.val = initial_scale
        self.velocity = 0.0
    ...
    def update_multiplicative(self, factor, dt):
        """Update value using momentum-based dynamics"""
        target_velocity = factor
        self.velocity = (self.velocity * 0.8) + (target_velocity * 0.2)
        step_change = np.clip(self.velocity * dt, -0.1, 0.1)
...
class AstroPhysicsSolver:
    """Physics-based equation solver with integer factorization support"""
    def __init__(self):
        self.variables = {}
    ...
    def _find_integer_factors(self, target_int, approx_p, approx_q, search_radius=1000000):
        """
        VALIDATED: Search around physics approximations for exact integer factors
        Returns factors closest to physics predictions that satisfy p*q = target_int
        """
        if target_int <= 0:
            return None
        
        sqrt_n = math.isqrt(target_int)
        print(f"[Factor Search] Target={target_int}, physics≈{approx_p:.2e}×{approx_q:.2e}")
        
        best_pair = None
        min_distance = float('inf')
        
        # Search AROUND BOTH approximations to find real factors
        for approx in [approx_p, approx_q]:
            ...
            for offset in range(search_radius):
                ...
                if target_int % candidate == 0:
                    other = target_int // candidate
                    factor_pair = sorted([candidate, other])
                    distance = abs(candidate - approx)
                    if distance < min_distance:
                        min_distance = distance
                        best_pair = factor_pair
                        print(f"[FOUND] {factor_pair[0]} × {factor_pair[1]} (dist={distance})")
                        return best_pair  # Return immediately for efficiency!
            
            # BACKWARD search (critical for balanced semiprimes)
    ...
    def solve(self, equation, steps=1000000, prefer_integers=False):
        """
        VALIDATED: Solve equations using physics-based optimization
        For factorization: equation = "p * q = N", prefer_integers=True
        """
        ...
        # Parse target value (support scientific notation and large integers)
        try:
        ...
        # Initialize variables near sqrt(N) for balanced factorization
        import re
        ...
        # Physics-based annealing in logarithmic space (stable convergence)
        for t in range(steps):
            ...
            if abs(error) < CONVERGENCE_THRESHOLD:
                print(f"[Converged] Step {t}, error={error:.2e}")
                break
            
            if t % 50000 == 0 and t > 0:
                print(f"[Progress] Step {t}/{steps}, error={error:.2e}")
            
            perturbation = 1.001
            log_perturb_delta = math.log10(perturbation)
            
            # Update each variable based on sensitivity
            for name in tokens:
                ...
                domain.val = orig  # Restore
                
                # Calculate sensitivity and apply force
        ...
        # VALIDATED: Integer snap with factor search
        if prefer_integers and target_int and len(tokens) == 2:
            if 'p' in tokens and 'q' in tokens:
                int_pair = self._find_integer_factors(
                    target_int, 
                    float_res['p'], 
                    float_res['q'], 
                    SEARCH_RADIUS
                )
                if int_pair and int_pair[0] > 1:  # Reject trivial p=1
...
def astro_factor_driver(n, timeout=60*30):
    """
    Use AstroPhysicsSolver.solve() on x * y = n and return YAFU-style factors.
    Never parse scientific-notation strings; work with numeric types only.
    """
    print(f"[*] Factoring {n} ({n.bit_length()} bits) with AstroPhysics...")

    solver = AstroPhysicsSolver()
    res = solver.solve(f"x * y = {n}", steps=1_000_000, prefer_integers=True)

    # Ensure we got both variables
    if not isinstance(res, dict) or 'x' not in res or 'y' not in res:
        print("[ASTRO] No solution from solver")
        return []

    x_approx = res['x']
    y_approx = res['y']

    # They should already be floats; NO string → int here
    # Python allows int(1.23e45) directly because it's a float, not a string. [web:1][web:8]
    if not isinstance(x_approx, (int, float)) or not isinstance(y_approx, (int, float)):
        print(f"[ASTRO] Unexpected types: x={type(x_approx)}, y={type(y_approx)}")
        return []

    # Convert to nearest integers
    p = int(round(x_approx))
    q = int(round(y_approx))
    ...
    if p <= 1 or q <= 1 or p * q != n:
        print(f"[ASTRO] Physics result invalid: p={p}, q={q}, p*q={p*q} != n")
        return []

    print(f"[ASTRO] Success: p={p}, q={q}")
    return [f"P{p.bit_length()} = {p}", f"Q{q.bit_length()} = {q}"]


# Integration functions
def pollard(n, limit=1000):
    """Pollard's rho algorithm for small factors"""
    x = 2; y = 2; d = 1; l = 0
...
def external_factorization(n, timeout=60*30):
    return astro_factor_driver(n, timeout=timeout)
...
def ranged_primorial(L, p=2):
    """Generate primorial for given level"""
    tmp = 2; c = 0
    while c < L:
        p = next_prime(p)
        tmp *= p
...
def SDC(W, candidates):
    """Square difference check for special forms"""
